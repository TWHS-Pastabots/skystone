...
@Autonomous
@Disabled
public class AutonBlueVisionTwoBlockPlatNEW extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        Stack<Double> xStackCopy;
        while (!isStarted()){
            if(runtime.milliseconds() > 50){
                xStack.push(skyStoneDetector.getScreenPosition().x);
                runtime.reset();
            }
            ...
            telemetry.addData("Vision Stack Size:", ""+xStack.size());
            int leftSum = 0;
            int midSum = 0;
            int rightSum = 0;
            String stoneConfig = "Not Detected Yet";
            xStackCopy = (Stack<Double>)xStack.clone();
            if(xStackCopy.size()>40) {
                for (int i = 0; i < 40; i++) {
                    double cur = xStackCopy.pop();
                    if (cur < 110) {
                        leftSum++;
                    } else if (cur < 200) {
                        midSum++;
            ...
            telemetry.addData("Detected Config:", stoneConfig);
            telemetry.update();
        }
        ...
        //Find the average x position of the skystone based on 40 readings
        int leftSum = 0;
        int midSum = 0;
        int rightSum = 0;
        String stoneConfig;
        for (int i = 0; i < 40; i++){
            double cur = xStack.pop();
            if(cur < 110){
                leftSum++;
            }
            else if(cur < 200){
        ...
        telemetry.addData("Config:", stoneConfig);
        telemetry.update();
        ...
        //Set the grabber arm and grips to correct positions
        ...
        //Drive left or right depending on where the skystone is
        if(stoneConfig.equals("Left")){
            gyroDriveCenter(DRIVE_SPEED, 0.6, 117, -4.0, 0, true);
        }
        else if(stoneConfig.equals("Right")){
            gyroDriveCenter(DRIVE_SPEED, 0.6, 70, 1.8, 0, true);
        }
        else{
            gyroDrive(DRIVE_SPEED, 49, 4.0, true, 5.0);
        }


        //Lower grabber onto block
        servo_grabber.setPosition(1.0);
        sleep(500);

        //Drag block back
        if(stoneConfig.equals("Right")){
            gyroDrive(DRIVE_SPEED, -14.0, 0, false, 3.0);
        }
        else {
            gyroDrive(DRIVE_SPEED, -7.0, 0, false, 2.0);
        }
        ...
        //Turn 90d left to face delivery area
        gyroTurn(TURN_SPEED, 90.0, 2.0);

        //Raise up the grabber halfway
        servo_grabber.setPosition(0.33);


        //THE FOLLOWING CODE MAY NEED TO BE REACTIVATED

        //Drive forward to delivery area
        if(stoneConfig.equals("Middle")){
            gyroDrive(DRIVE_SPEED, 122.5, 90,true, 10.0);
        }
        else if(stoneConfig.equals("Right")){
            gyroDrive(DRIVE_SPEED, 140, 90,true, 10.0);
        }
        else{
            gyroDrive(DRIVE_SPEED, 115, 90,true, 10.0);
        }

        //Turn 90d right to face platform
        gyroTurn(TURN_SPEED, 0.0, 3.0);

        //Drive forward to be up against platform
        gyroDrive(DRIVE_SPEED, 7.0, 0, false, 2.0);


        //Drop the block onto the platform
        servo_grabber.setPosition(1.0);
        servo_gripRight.setPosition(0.0);
        servo_gripLeft.setPosition(1.0);
        sleep(300);
        servo_grabber.setPosition(0.2);
        sleep(500);

        //Drive back away from platform
        gyroDrive(DRIVE_SPEED, -3.0, 0.0, false, 0.30);
        servo_grabber.setPosition(0.33);

        //Turn 90d to the right to face back to quarry
        gyroTurn(TURN_SPEED, -90.0, 2.0);

        //Drive forward to quarry
        if(stoneConfig.equals("Middle")){
            gyroDrive(DRIVE_SPEED, 170, -90, true, 10.0);
        }
        else if(stoneConfig.equals("Right")){
            gyroDrive(DRIVE_SPEED, 62, -90, true, 10.0);
        }
        else{
            gyroDrive(DRIVE_SPEED, 160, -90, true, 10.0);
        }
        ...
        /*
        if(stoneConfig.equals("Right")){
            encoderDriveCenter(0.5, -9.0, 3.0, false);
            gyroDrive(0.4, 5.0, -90, true, 1.0);

            servo_gripRight.setPosition(1.0);
            servo_gripLeft.setPosition(0.0);
            sleep(1000);

            gyroTurn(TURN_SPEED, 0, 1.5);

            gyroDrive(DRIVE_SPEED, 10.0, 0.0, true, 1.5);

            gyroTurn(TURN_SPEED, 90.0, 1.5);

            gyroDrive(1.0, 62, 90, true, 10.0);
            ...
            gyroDrive(DRIVE_SPEED, -10.0, 90, false, 10.0);
        }

         */

        if(!(stoneConfig.equals("Right"))) {
            //Turn 90d left to face blocks
            gyroTurn(TURN_SPEED, 0.0, 3.0);

            //Drive forward to block
            gyroDrive(DRIVE_SPEED, 11.0, 0, false, 10.0);

            //Lower grabber onto block
            servo_grabber.setPosition(1.0);
            sleep(500);
            servo_gripRight.setPosition(0.5);
            servo_gripLeft.setPosition(0.5);

            //Drag block backwards
            gyroDrive(DRIVE_SPEED, -5.0, 0, false, 10.0);

            //Grip the block
            servo_gripRight.setPosition(1.0);
            servo_gripLeft.setPosition(0.0);
            sleep(750);
            //Raise arm to halfway up
            servo_grabber.setPosition(0.33);

            //Turn 90d left to face delivery area
            gyroTurn(TURN_SPEED, 90.0, 3.0);

            //Drive forward to delivery area, but only slightly into it
            if (stoneConfig.equals("Left")) {
                gyroDrive(DRIVE_SPEED, 118, 90, true, 3.0);
            } else if (stoneConfig.equals("Middle")) {
                gyroDrive(DRIVE_SPEED, 125, 90, true, 3.0);
            }

            //Let go of the block and raise arm
            servo_gripRight.setPosition(0.0);
            servo_gripLeft.setPosition(1.0);
            servo_grabber.setPosition(0.33);
            sleep(750);

            //Drive backwards to park over the line
            gyroDrive(DRIVE_SPEED, -20.0, 90, false, 10.0);
        }
