...
@Autonomous
@Disabled
public class AutonTestOutDistance extends LinearOpMode {
    ...
    private DistanceSensor sensor_fDist;
    ...
    public void runOpMode() {
        ...
        sensor_fDist = hardwareMap.get(DistanceSensor.class, "sensor_fDist");
        ...
        //                                                     |
        //_____________________________________________________|
        // Note: Reverse movement is obtained by setting a negative distance (not speed), negative distance on H-Drive is toward the wall and you must say if you are going right
    ...
    private void gyroDriveCenterDistance(double speed, double cSpeed,
                           double distance, double cDistance, boolean goRight,
                           double angle,
                           boolean rampDown,
        ...
        if (opModeIsActive()) {
            ...
            centerDirectionCoeff = goRight ? 1.0 : -1.0;

            // Determine new target position, and pass to motor controller
            newLeftTarget = motor_left.getCurrentPosition() + (int)(Math.abs(distance) * DRIVE_COUNTS_PER_INCH);
            newRightTarget = motor_right.getCurrentPosition() + (int)(Math.abs(distance) * DRIVE_COUNTS_PER_INCH);
            newCenterTarget = sensor.getDistance(DistanceUnit.INCH) + cDistance;
            ...
            while (opModeIsActive() && (
                    ...
                    ||  (Math.signum(cDistance)>0) ? sensor.getDistance(DistanceUnit.INCH)<newCenterTarget : sensor.getDistance(DistanceUnit.INCH)>newCenterTarget )) {
                ...
                if((Math.signum(cDistance)>0) ? sensor.getDistance(DistanceUnit.INCH)>newCenterTarget : sensor.getDistance(DistanceUnit.INCH)<newCenterTarget)
                    centerSteer = 0.0;
                ...
                motor_left.setPower(leftSpeed * Math.signum(distance));
                motor_right.setPower(rightSpeed* Math.signum(distance));
                motor_center.setPower(centerSpeed * centerDirectionCoeff);
